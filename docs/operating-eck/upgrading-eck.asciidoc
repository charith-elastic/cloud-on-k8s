...
Before upgrading, refer to the <<release-notes-{eck_version}, release notes>> to ensure that there are no breaking changes in the release that could affect your current setup. The <<release-highlights-{eck_version},release highlights document>> includes more information about any breaking changes in each release and possible mitigation steps.
